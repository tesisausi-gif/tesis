...
import {
  ...
  CheckCircle2, Clock, User, Banknote, ArrowLeftRight, History, Loader2, Search, ExternalLink, AlertTriangle,
} from 'lucide-react'
...
function TabPagosTecnicos({ pendientes, realizados, highlightId, cobrosCliente }: { pendientes: PendientePagoTecnico[]; realizados: PagoTecnicoRegistrado[]; highlightId?: number | null; cobrosCliente: CobroClienteRegistrado[] }) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  ...
  const handlePagar = () => {
    ...
    const clienteCobrado = cobrosCliente.some(c => c.id_incidente === pagarDialog.id_incidente)
    if (!clienteCobrado) { toast.error('Primero debés registrar el cobro al cliente antes de pagar al técnico'); return }
    const err = validarMetodoPago(formPago)
    if (err) { toast.error(err); return }
    startTransition(async () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {vista === 'pendientes' && (
        <div>
          {pendientes.length === 0 ? (
            ...
            <div className="grid gap-3">
              {pendientesPag.map(p => {
                const clienteCobrado = cobrosCliente.some(c => c.id_incidente === p.id_incidente)
                return (
                  <div
                    key={p.id_presupuesto}
                    ref={el => { if (el) highlightRefs.current.set(p.id_incidente, el); else highlightRefs.current.delete(p.id_incidente) }}
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {!clienteCobrado && (
                                <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200 text-xs flex items-center gap-1">
                                  <AlertTriangle className="h-3 w-3"/>Cliente sin cobrar
                                </Badge>
                              )}
                            </div>
                            <p className="text-sm text-gray-600 truncate">Incidente #{p.id_incidente} — {p.descripcion_problema}</p>
      ...
      {(() => {
        const clienteCobrado = pagarDialog ? cobrosCliente.some(c => c.id_incidente === pagarDialog.id_incidente) : true
        return (
          <Dialog open={pagarDialog !== null} onOpenChange={o => !o && setPagarDialog(null)}>
            <DialogContent className="max-w-md">
              ...
              <div className="py-2 space-y-3">
                {!clienteCobrado && (
                  <div className="flex items-start gap-2.5 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-sm text-amber-800">
                    ...
                    <p>El cliente aún no fue cobrado por este incidente. Primero registrá el cobro al cliente en la pestaña <strong>Cobros a Clientes</strong>.</p>
                  </div>
                )}
              ...
              <DialogFooter>
                ...
                <Button onClick={handlePagar} disabled={isPending || !formPago.metodo || !clienteCobrado} className="bg-green-600 hover:bg-green-700 gap-2">
